...
会员卡是否支持跨店使用（适用场景：订单消费、会员卡储值、会员卡整单退款）)</span>
          ...
          <el-form-item>
            <el-button  @click="resetForm('cardTypeFormRef')">重置</el-button>
            <el-button type="primary" @click="onSubmit('cardTypeFormRef')">保存</el-button>
          </el-form-item>
        </el-form>
      </el-card>
    </div>
...
export default {
  data () {
    return {
      ...
      cardTypeForm: {
        ...
        give_money: '1', // 编辑赠送金额
        // eslint-disable-next-line camelcase
        money_state: '1', // 是否支持储值
        // eslint-disable-next-line camelcase
        money_type: '0', // 是否支持储值消费
        // eslint-disable-next-line camelcase
        score_state: '1', // 积分支付
        // eslint-disable-next-line camelcase
        coupon_state: '1', // 优惠券支付
        // eslint-disable-next-line camelcase
        shop_state: '1', // 支持跨店使用
        // eslint-disable-next-line camelcase
        sale_state: '0', // 价格设置
        // eslint-disable-next-line camelcase
        password_state: '0', // 支付密码设置
        // eslint-disable-next-line camelcase
        member_score_state: '1', // 会员卡支持积分
        // eslint-disable-next-line camelcase
        score_rule: 1, // 会员卡积分规则
        // eslint-disable-next-line camelcase
        member_rule: '', // 会员卡升级规则
        // eslint-disable-next-line camelcase
        internet_state: '0', // 网点注册默认类型
        // eslint-disable-next-line camelcase
        score_grade_state: '1', // 是否使用卡等级积分
        // eslint-disable-next-line camelcase
        member_grade_state: '1', // 是否使用会员卡等级
        state: '1', // 是否启用
        remark: '' // 备注
      },
      ruleOptions: [
  ...
  methods: {
    ...
    scoreState () {
      ...
      console.log(this.cardTypeForm)
    },
    onSubmit (formName) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          const { data: res } = await this.$http.post('mapi/memberTypeAdd', this.cardTypeForm)
