<template>
    <div>
      ...
      <!-- <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>会员卡添加</el-breadcrumb-item>
      </el-breadcrumb> -->
      <!-- 卡片视图区 -->
      <el-card>
        <el-form :model="cardForm" :rules="cardFormRules" ref="cardFormRef" label-width="130px">
          ...
          <el-form-item label="所属门店" prop="shop_id">
            <el-select v-model="cardForm.shop_id" placeholder="选择门店">
              <el-option
                v-for="item in shopOtions"
                :key="item.id"
                :label="item.title"
                :value="item.id">
              </el-option>
          ...
          <el-form-item label="会员卡积分">
            <el-input v-model="cardForm.score"></el-input>
          </el-form-item>
          <el-form-item label="会员卡地址">
            <el-input type="textarea" :rows="2" v-model="cardForm.address"></el-input>
          </el-form-item>
          <el-form-item label="备注">
            <el-input type="textarea" :rows="4" v-model="cardForm.remark"></el-input>
          </el-form-item>
          <el-form-item label="开卡成功短信通知">
            <el-switch v-model="cardForm.note_inform" active-value="1" inactive-value="0"></el-switch> <span class="tips">(会员卡是否支持储值)</span>
          </el-form-item>
           <el-form-item>
            <el-button  @click="resetForm('cardFormRef')">重置</el-button>
            <el-button type="primary" @click="onSubmit('cardFormRef')" v-no-click>保存</el-button>
          </el-form-item>
        </el-form>
      </el-card>
    </div>
...
export default {
  data () {
    ...
    return {
      ...
      cardForm: {
        ...
        shop_id: '', // 所属门店
        // eslint-disable-next-line camelcase
        member_type_id: '', // 会员卡类型
        // eslint-disable-next-line camelcase
        member_grade_id: '0', // 会员卡等级
        password: '', // 交易密码
        // eslint-disable-next-line camelcase
        start_time: '', // 有效期开始时间
        // eslint-disable-next-line camelcase
  ...
  methods: {
    async getCardInfo () {
      const { data: res } = await this.$http.get('mapi/memberMemberAdd')
      console.log(res)
      if (res.code !== 200) return this.$message.error('获取卡信息失败！')
      this.cardStepOptions = res.list.grade
      this.shopOtions = res.list.shop
      this.cardTypeOptions = res.list.type
    },
    getNowTime () {
      // const date = new Date()
    ...
    onSubmit (formName) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          const { data: res } = await this.$http.post('mapi/memberMemberAdd', this.cardForm)
