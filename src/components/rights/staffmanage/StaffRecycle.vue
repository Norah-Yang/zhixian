<template>
    <div>
      ...
      <!-- <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>员工回收站</el-breadcrumb-item>
      </el-breadcrumb> -->
      <!-- 卡片视图区域 -->
      <el-card>
         <el-row class="mb10">
           <el-col :span="24">
             ...
             <el-button type="primary" size="mini" @click="staffList">列表 <i class="el-icon-s-order"></i></el-button>
           </el-col>
         </el-row>
         <el-table :data="tableData" height="680" border style="width: 100%">
           ...
           <el-table-column label="操作" width="80">
             <template slot-scope="scope">
               <el-button type="primary" size="mini" @click="recover(scope.row, scope.$idnex)">恢复</el-button>
             </template>
           </el-table-column>
         </el-table>
         <el-pagination
...
export default {
  ...
  methods: {
    ...
    // 恢复
    async recover (row, index) {
      const { data: res } = await this.$http.get('mapi/recoverEmployee', { params: { id: row.id } })
      console.log(res)
      if (res.code !== 200) return this.$message.error('员工恢复失败！')
      this.$message.success('员工恢复成功！')
