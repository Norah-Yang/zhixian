      ...
      <el-card>
         <el-row class="mb10">
           <el-col :span="24">
             ...
             <el-button type="primary" size="mini" @click="clienList">列表 <i class="el-icon-s-order"></i></el-button>
           </el-col>
         </el-row>
         <el-table :data="tableData" height="680" border style="width: 100%">
           ...
           <el-table-column label="操作" width="80">
             <template slot-scope="scope">
               <el-button type="primary" size="mini" @click="backBtn(scope.row, scope.$index)">恢复</el-button>
             </template>
           </el-table-column>
         </el-table>
         <el-pagination
...
export default {
  ...
  methods: {
    async getClientList () {
      const { data: res } = await this.$http.post('mapi/clientClientRecycle', this.queryInfo)
      console.log(res)
    ...
    backBtn (row, index) {
      console.log(row)
      this.$confirm('确定恢复此客户吗?', '温馨提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        const { data: res } = await this.$http.get('mapi/clientClientRecycleRestore', { params: { id: row.id } })
